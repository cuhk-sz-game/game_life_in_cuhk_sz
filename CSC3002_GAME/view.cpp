...
void View::keyStudyTogether(QKeyEvent *event){
    emit events("studyTogether");
    switch(event->key()){
    case Qt::Key_1:
        if(player.GetEnergy()>=10 && player.GetSingle() == false){
            player.SetEnergy(player.GetEnergy()-10);
            player.SetIQ(player.GetIQ()+3);
            player.SetEQ(player.GetEQ()+1);
            player.SetCharm(player.GetCharm()+1);
            player.SetLike(player.GetLike()+5);
        }
        else {
            events("lackenergy");
        }
...
void View::keyprof2(QKeyEvent *event){
    switch(event->key()){
    case Qt::Key_1:
        if(player.GetEnergy()>=15 && player.GetIQ()>=150){
            player.SetEnergy(player.GetEnergy()-15);
...
void View::keyGymTogether(QKeyEvent *event){
    emit events("gymTogether");
    switch(event->key()){
    case Qt::Key_1:
        if(player.GetEnergy()>=10 && player.GetSingle() == false){
            player.SetEnergy(player.GetEnergy()-10);
...
void View::keyEatTogether(QKeyEvent *event){
    emit events("eatTogether");
    switch(event->key()){
    case Qt::Key_1:
        if(player.GetMoney()>=20 && player.GetSingle() == false){
            player.SetEnergy(player.GetEnergy()+15);
            player.SetMoney(player.GetMoney()-20);
            player.SetEat(player.GetEat()+1);
        }
...
void View::keyMovie(QKeyEvent *event){
    emit events("movie");
    switch(event->key()){
    case Qt::Key_1:
        if(player.GetEnergy()>=5 && player.GetMoney()>=20 && player.GetSingle() == false && player.GetLike() >= 20){
            player.SetEnergy(player.GetEnergy()-5);
            player.SetMoney(player.GetIQ()-20);
            player.SetEQ(player.GetEQ()+1);
            player.SetLike(player.GetLike()+10);
        }else{
            emit events("lackMovie");
        }
        break;
    case Qt::Key_2:
        SetStatus("main");
...
void View::keyDating(QKeyEvent *event){
    switch(event->key()){
    case Qt::Key_1:
        keyStudyTogether(event);
        break;
    case Qt::Key_2:
        keyEatTogether(event);
        break;
    case Qt::Key_3:
        keyGymTogether(event);
        break;
    case Qt::Key_4:
        keyMovie(event);
        break;
